...
<body>
    <div class="container">
        <div class="header">
            ...
            <a href="https://github.com/nrbrook/nudel-downloader" class="github-link" target="_blank" title="View on GitHub">
    ...
    <script>
        ...
        function animateCardSelection(cards) {{
            ...
            function showCard(card, index) {{
                ...
                const directions = ['translateX(-100%)', 'translateX(100%)', 'translateY(-100%)', 'translateY(100%)', 'scale(0.5)', 'rotateY(90deg)'];
                const randomDir = directions[Math.floor(Math.random() * directions.length)];

                cardEl.style.transform = randomDir;
                cardEl.style.opacity = '0';
                container.appendChild(cardEl);

                // Trigger animation
                requestAnimationFrame(() => {{
                    cardEl.style.transition = 'transform 0.15s ease-out, opacity 0.15s ease-out';
                    cardEl.style.transform = 'translateX(0) translateY(0) scale(1) rotateY(0)';
